...
import cn.com.dhcc.query.creditpersonqueryinterface.webservice.batchquery.bean.dto.GetBatchQueryStatusDto;
...
public class BatchGetQueryStatusTask extends Thread {
	...
	public BatchGetQueryStatusTask(CpqBatchinfo cpqBatchinfo) {
	...
	public void run() {
		...
		GetBatchQueryStatusDto batchQueryStatusDto = new GetBatchQueryStatusDto(creditUser.getCcdept(), creditUserName, creditUserPasswd, batchQueryFileName);
		BatchResultMsgBean sendBatchQueryMessage = queryService.getBatchQueryStatus(batchQueryStatusDto);
		if(Objects.equal(sendBatchQueryMessage.getCode(), "00000000")){
			//人行处理完成，更新redis
			CpqBatchinfo batch = batchinfoService.findById(cpqBatchinfo.getId());
			batch.setStatus(Constant.BATCH_STATUS_GET_RESULT);
			batch.setUpdatetime(new Date());
			BatchInRedis batchInRedis = new BatchInRedis(batch.getId(), Constant.BATCH_STATUS_GET_RESULT, Constant.BATCH_ROWSTATUS_UNTREATED, new Date());
