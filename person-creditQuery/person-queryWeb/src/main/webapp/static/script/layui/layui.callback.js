       ...
       */
      function deleteBtnCallback(returnObj,idArr,obj){
          var dataTable = returnObj.dataTable,
              searchData= returnObj.searchData
              $.post(obj.delUrl+idArr,function(data){
                json = strToJson(data);
                  layerAlert(json.msg,function(){
                     // reloadGrid(dataTable,searchData)
                     reloadPage();
                  })
              });
           /*   if(obj.tabs){
                  var del =obj.listObj.tabs[0].delUrl
                  $.post(del+idArr,function(data){
                    json = strToJson(data);
                      layerAlert(json.msg,function(){
                         reloadGrid(dataTable,searchData)
                      })
      ...
      function submitTabsMsg(result,iframe){ 
              ...
              var tabs =parent.listObj.tabs;
               parent["tabsData_"+index] = result.objectData;
