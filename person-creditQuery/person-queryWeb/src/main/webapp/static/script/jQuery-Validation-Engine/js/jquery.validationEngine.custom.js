...
function checkassocbsnssData(field, rules, i, options) {
    
    var arr = returnObject(field, options, "checkassocbsnssData");
    ...
    var idCardLength = idCard.length;
    // 取到查询原因
    var qryReason = arr[0].qryReason;
    if(!qryReason){
        return true;
    }
    qryReason = arr[0].qryReason.value;
    // 校验
    if(qryReason == '01'/*如果是贷后，则关联业务数据必填*/){
        if(idCard == ""){
            return arr[2].alertText;
        }
   }
