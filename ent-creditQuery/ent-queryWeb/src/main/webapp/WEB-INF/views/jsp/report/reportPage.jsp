...
<%@page import="cn.com.dhcc.creditquery.ent.queryweb.util.UserAttrUtil"%>
<%@page import="cn.com.dhcc.creditquery.ent.query.bo.queryconfig.CeqUserAttrBo"%>
...
CeqUserAttrBo userAttr = UserAttrUtil.findCeqUserAttrBySystemUserId(userName);
if(null == userAttr){
	userAttr = new CeqUserAttrBo();
}
%>
...
<body oncontextmenu='return false;' ondragstart='return false;'
	...
	<script LANGUAGE="JavaScript"> 
 	...
 	var printattchObj = {};
 	//信用报告ID
 	var recordId = '<%=(String)request.getAttribute("recordId")%>';
		...
		var report = report || {} ;
			report.index = {
				init : function() {
					...
					// 隐藏保存按钮
					<%if ("1".equals(CeqConfigUtil.getSavePolicy().trim()) || "1".equals(userAttr.getSavePermit())) {%>
								$(".btnSave").hide();
						    <%}
					if ("1".equals(CeqConfigUtil.getPrintPolicy().trim()) || "1".equals(userAttr.getPrintPermit())) {%>
								$(".btnPrint").hide();
						    <%}%>
					},
				mainIframe : function() {
					...
					// body.find("td").removeAttr("bgcolor");
					// 设置水印图片
					<%if("1".equals(CeqConfigUtil.getWatermarkPolicy())) {%>
